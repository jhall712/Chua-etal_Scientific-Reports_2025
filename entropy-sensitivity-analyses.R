...
metadata<-read.csv("sensor-subsample-description-final-with-MOT.csv")
metadata<-select(metadata, c("ID", "preterm", "sits_unsupported"))
...
df<-df%>%mutate(
  ...
  subset(exclude==0)%>%
  select(-exclude)%>%
  filter(sits_unsupported==1)
  
library(tidyr) 
#Compute complexity index (all) and at each frequency band
df_CI_bands<-pivot_longer(df, cols=c(6:55), names_to="Scale", values_to="Entropy")%>%
  mutate(
...
setwd("XXXXX")
write.csv(M_modcompare, file="model-building-CI-sensitivity.csv")
...
write.csv(C_modcompare, file="model-building-C-sensitivity.csv")
anova(C3)
...
anova(B3)
write.csv(B_modcompare, file="model-building-B-sensitivity.csv")
...
write.csv(A_modcompare, file="model-building-A-sensitivity.csv")
anova(A3)
...
write.csv(D_modcompare, file="model-building-D-sensitivity.csv")
anova(D3)
...
write.csv(E_modcompare, file="model-building-E-sensitivity.csv")
anova(E3)
...
write.csv(emm.alpha.plot, "EMM-alpha-sensitivity.csv")
write.csv(emm.beta.plot, "EMM-beta-sensitivity.csv")
write.csv(emm.gamma.plot, "EMM-gamma-sensitivity.csv")
write.csv(emm.delta.plot, "EMM-delta-sensitivity.csv")
write.csv(emm.theta.plot, "EMM-theta-sensitivity.csv")

write.csv(contrasts.alpha.P, "contrasts-Phase-alpha-sensitivity.csv")
write.csv(contrasts.beta.P, "contrasts-Phase-beta-sensitivity.csv")
write.csv(contrasts.gamma.P, "contrasts-Phase-gamma-sensitivity.csv")

write.csv(contrasts.beta.PT, "contrasts-Preterm-beta-sensitivity.csv")
write.csv(contrasts.gamma.PT, "contrasts-Preterm-gamma-sensitivity.csv")
write.csv(contrasts.delta.PT, "contrasts-Preterm-delta-sensitivity.csv")
write.csv(contrasts.theta.PT, "contrasts-Preterm-theta-sensitivity.csv")
write.csv(contrasts.CI.PT, "contrasts-Preterm-CI-sensitivity.csv")
